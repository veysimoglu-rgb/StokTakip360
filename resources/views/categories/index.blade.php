<x-app-layout>
    ...
    <div class="flex items-center justify-between mb-4">
        <form method="GET" class="flex gap-2">
            <x-text-input type="text" name="q" value="{{ request('q') }}" placeholder="Kategori ara..." class="w-64" />
            <x-secondary-button type="submit">Ara</x-secondary-button>
        </form>
        <a href="{{ route('categories.create') }}">
            <x-primary-button>+ Yeni Kategori</x-primary-button>
        </a>
    </div>
                ...
                @forelse ($categories as $category)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right space-x-2">
                            <a href="{{ route('categories.edit', $category) }}" class="text-indigo-600 hover:underline">Düzenle</a>
                            <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline" onsubmit="return confirm('Silinsin mi?')">
                                @csrf @method('DELETE')
