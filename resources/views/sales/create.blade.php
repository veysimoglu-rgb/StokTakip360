<x-app-layout>
    <x-slot name="title">Yeni Satış</x-slot>

    <div
        x-data="{
            items: [{ product_id: '', quantity: 1, unit_price: 0 }],
            products: {{ Js::from($products->map(fn ($p) => ['id' => $p->id, 'name' => $p->name, 'price' => (float) $p->sale_price, 'currency' => $p->currency, 'stock' => $p->current_stock])) }},
    ...
    >
        <form method="POST" action="{{ route('sales.store') }}" class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow p-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="sale_date" type="date" name="sale_date" value="{{ now()->toDateString() }}" class="w-full" />
                    </div>
                </div>
                <div class="mt-4">
            ...
            <div class="flex gap-3">
                <x-primary-button type="submit">Satışı Kaydet</x-primary-button>
                <a href="{{ route('sales.index') }}" class="text-sm text-gray-600 self-center hover:underline">Vazgeç</a>
            </div>
        </form>
    </div>
</x-app-layout>
