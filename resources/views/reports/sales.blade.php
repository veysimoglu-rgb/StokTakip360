<x-app-layout>
    <x-slot name="title">Satış Raporu</x-slot>
    ...
    <form method="GET" class="flex flex-wrap gap-2 mb-6 items-end">
        ...
        <div class="ms-auto flex gap-2">
            ...
            <a href="{{ route('reports.sales.print', request()->query()) }}" target="_blank">
    ...
    @forelse ($totalsByCurrency as $currency => $row)
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-3">
            <div class="bg-white rounded-lg shadow p-5">
                <p class="text-sm text-gray-500">Toplam Satış ({{ $currency }})</p>
    ...
    @empty
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-3">
            <div class="bg-white rounded-lg shadow p-5">
                <p class="text-sm text-gray-500">Toplam Satış (TL)</p>
    ...
    @endforelse
    <p class="text-xs text-gray-400 mb-6">İptal edilmiş satışlar listede görünür ancak yukarıdaki toplamlara dahil edilmez.</p>
            ...
            <tbody class="divide-y">
                @forelse ($sales as $sale)
                    <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('sales.show', $sale) }}'">
                        ...
                        <td class="hidden sm:table-cell px-4 py-3">{{ $sale->paymentTypeLabel() }}</td>
                        <td class="px-4 py-3">
                            @if ($sale->isCancelled())
                        ...
                        <td class="hidden sm:table-cell px-4 py-3">{{ $sale->currency }}</td>
                        <td class="px-4 py-3 text-right font-medium">{{ \App\Support\Currency::format($sale->total, $sale->currency) }}</td>
                        <td class="hidden sm:table-cell px-4 py-3 text-right text-green-600">{{ \App\Support\Currency::format($sale->paid_amount, $sale->currency) }}</td>
                        <td class="hidden sm:table-cell px-4 py-3 text-right text-red-600">{{ \App\Support\Currency::format($sale->remaining(), $sale->currency) }}</td>
                        <td class="hidden sm:table-cell px-4 py-3">{{ $sale->due_date?->format('d.m.Y') ?? '-' }}</td>
                    </tr>
                @empty
                    <tr><td colspan="10" class="px-4 py-6 text-center text-gray-500">Seçilen dönemde satış bulunamadı.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
