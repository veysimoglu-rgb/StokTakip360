    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-lg">
        <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-4">
            ...
            <div>
                ...
                <x-text-input id="name" name="name" value="{{ old('name', $customer->name) }}" class="w-full" required autofocus />
            ...
            <div>
                ...
                <x-text-input id="email" type="email" name="email" value="{{ old('email', $customer->email) }}" class="w-full" />
                <x-input-error :messages="$errors->get('email')" class="mt-1" />
            </div>
            <div>
                <x-input-label for="tax_no" value="Vergi No" />
                <x-text-input id="tax_no" name="tax_no" value="{{ old('tax_no', $customer->tax_no) }}" class="w-full" />
            </div>
            <div>
                <x-input-label for="address" value="Adres" />
                <x-textarea-input id="address" name="address" rows="3">{{ old('address', $customer->address) }}</x-textarea-input>
            </div>

            <label class="inline-flex items-center">
                <input type="checkbox" name="active" value="1" @checked(old('active', $customer->active)) class="rounded border-gray-300 text-indigo-600">
