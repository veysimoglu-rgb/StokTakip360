<x-app-layout>
    <x-slot name="title">Marka Düzenle</x-slot>
    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-lg">
        <form method="POST" action="{{ route('brands.update', $brand) }}" class="space-y-4">
            ...
            <div>
                ...
                <x-text-input id="name" name="name" value="{{ old('name', $brand->name) }}" class="w-full" required autofocus />
                <x-input-error :messages="$errors->get('name')" class="mt-1" />
            </div>

            <label class="inline-flex items-center">
                <input type="checkbox" name="active" value="1" @checked(old('active', $brand->active)) class="rounded border-gray-300 text-indigo-600">
